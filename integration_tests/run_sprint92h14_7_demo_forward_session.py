...
- default mode sends no real order
- MT5 Demo execution requires explicit --demo-forward
- no True-OOS access
"""
...
def predecessor_journal_path_for(
    ...
    return (
        ...
        / "shadow_data"
        / "live"
        / "sprint92h14_7_demo_forward_no_predecessor"
        / safe_symbol_name(symbol)
        / "shadow_positions.jsonl"
    )
...
def new_stats():
    return {
        ...
        "demo_execution_attempts": 0,
        "demo_execution_successes": 0,
        "demo_execution_blocks": 0,
    }
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Explicitly enable MT5 DEMO execution. "
            "Default remains no-send Shadow mode."
        ),
    )
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Maximum Demo broker execution attempts. "
            "0 means no explicit attempt limit."
        ),
    )

    parser.add_argument(
        "--demo-probe-min-volume",
        action="store_true",
        help=(
            "For controlled Demo probing only: "
            "cap execution volume to broker volume_min."
        ),
    )
    ...
    if args.max_demo_attempts < 0:
        raise RuntimeError(
            "MAX_DEMO_ATTEMPTS_MUST_BE_NON_NEGATIVE"
        )
    ...
    MT5_RECONNECT_COOLDOWN_SECONDS = 5.0

    runtime_test_actual_disconnect_cycles_remaining = int(
        os.environ.get(
            "MSS_H14_7_TEST_ACTUAL_MT5_DISCONNECT_CYCLES",
            "0",
        )
    )
    ...
    try:
        ...
        if args.demo_forward:
            demo_account = mt5.account_info()

            if demo_account is None:
                raise RuntimeError(
                    "DEMO_FORWARD_ACCOUNT_INFO_UNAVAILABLE"
                )
            ...
            if not (
                ...
                raise RuntimeError(
                    "DEMO_FORWARD_NON_DEMO_ACCOUNT_BLOCKED"
                )
        ...
        with ThreadPoolExecutor(
            ...
            while True:
                ...
                if (
                    runtime_test_actual_disconnect_cycles_remaining
                    > 0
                ):
                    runtime_test_actual_disconnect_cycles_remaining -= 1
                    ...
                    print(
                        "H14_7_TEST_ACTUAL_MT5_DISCONNECT_ACTIVE",
                        runtime_test_actual_disconnect_cycles_remaining,
                    )
                ...
                # =========================================
                # H14.7 AUTOMATIC MT5 SESSION RECOVERY.
                #
                # Recover ONLY an actual MT5 failure.
                # Do not react to the deterministic H14.6
                # disconnect fault-injection hook, which
                # changes effective_terminal_connected only.
                #
                # Recovery remains fail-safe:
                # - new entries remain blocked while invalid
                # - shutdown old Python/terminal session
                # - bounded reinitialize attempts
                # - terminal/account revalidation
                # - Demo account revalidation when enabled
                # =========================================

                actual_mt5_disconnect = bool(
                    not initialized
                    or
                    terminal_now is None
                    or
                    account_now is None
                    or
                    not terminal_connected
                )

                if actual_mt5_disconnect:
                    reconnect_now = (
                        time.monotonic()
                    )
                    ...
                    reconnect_due = (
                        reconnect_last is None
                        or
                        (
                            reconnect_now
                            - float(reconnect_last)
                        )
                        >=
                    ...
                    if reconnect_due:
                        ...
                        if reconnect_ok:
                            ...
                            terminal_after_reconnect = (
                                mt5.terminal_info()
                            )

                            account_after_reconnect = (
                                mt5.account_info()
                            )

                            connected_after_reconnect = bool(
                                terminal_after_reconnect
                                is not None
                                and
                                getattr(
                            ...
                            demo_after_reconnect_valid = True
                            ...
                            if (
                                ...
                                print(
                                    "MT5_RUNTIME_RECONNECTED_SERVER",
                                    getattr(
                                        account_now,
                                        "server",
                                        None,
                                ...
                                if (
                                    connected_after_reconnect
                                    and
                                    not demo_after_reconnect_valid
                                ):
                                    reconnect_error = (
                                        "NON_DEMO_ACCOUNT_AFTER_RECONNECT"
                                    )
                                else:
                                    reconnect_error = (
                                        "TERMINAL_OR_ACCOUNT_INVALID_AFTER_RECONNECT"
                                    )
                        ...
                        if not bool(
                            ...
                            if (
                                ...
                                if not final_policy.allowed:
                                    ...
                                    # =====================================
                                    # H14.7 DEMO BROKER TRANSACTION.
                                    #
                                    # In Demo mode there MUST be no Shadow
                                    # open journal event before broker
                                    ...
                                    if args.demo_forward:
                                        ...
                                        if not risk_result.valid:
                                            ...
                                            global_stats[
                                                "demo_execution_blocks"
                                            ] += 1

                                            demo_execution_runtime[
                                                "last_reason"
                                            ] = (
                                                "DEMO_RISK_SIZE_BLOCKED:"
                                                f"{risk_result.reason}"
                                            )

                                            print(
                                                "DEMO_FORWARD_BLOCKED",
                                                selected_symbol,
                                                demo_execution_runtime[
                                                    "last_reason"
                                                ],
                                            ...
                                            if args.demo_probe_min_volume:
                                                ...
                                                if probe_symbol_info is None:
                                                    raise RuntimeError(
                                                        "DEMO_PROBE_SYMBOL_INFO_"
                                                        "UNAVAILABLE"
                                                    )
                                                ...
                                                print(
                                                    "DEMO_PROBE_MIN_VOLUME_ACTIVE",
                                                    True,
                                                )

                                                print(
                                                    "DEMO_PROBE_BROKER_MIN_VOLUME",
                                                    broker_min_volume,
                                                )
                                            ...
                                            if demo_result.valid:
                                                ...
                                                global_stats[
                                                    "demo_execution_successes"
                                                ] += 1

                                                shadow_open_permitted = True

                                                print(
                                                    "DEMO_FORWARD_ORDER_EXECUTED",
                                                    selected_symbol,
                                                    demo_result.order_ticket,
                                                    demo_result.deal_ticket,
                                                )

                                                print(
                                                    "DEMO_FORWARD_REQUESTED_PRICE",
                                                    demo_result
                                                    .requested_price,
                                                )

                                                print(
                                                    "DEMO_FORWARD_FILL_PRICE",
                                                    demo_result
                                                    .fill_price,
                                                )

                                                print(
                                                    "DEMO_FORWARD_SLIPPAGE",
                                                    demo_execution_runtime[
                                                        "last_slippage"
                                                    ],
                                                )
                                    ...
                                    # mirror in Demo mode.
                                    #
                                    # Broker failure => this block is never
                                    # entered.
                                    # =====================================
